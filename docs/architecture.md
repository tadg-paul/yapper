<!-- Version: 0.8 | Last updated: 2026-07-14 -->
...
`SpeechEngineCapabilities.supportsIPA`, governed by `AC46.11` and `AC47.11`-`AC47.12`, records support for Yapper's configured IPA contract; it does not assume that every provider accepts the same wire syntax. The adapter owns any engine-specific encoding. Yapper declares support; FAL and OpenAI do not. Future engines, including F5, declare the capability when registered. Configuration resolution merges Unicode case-insensitive, whole-term substitutions, superimposes the selected engine's exceptions, and chooses IPA or its optional phonetic fallback before engine chunking. Unsupported IPA without a fallback leaves the source term unchanged.
